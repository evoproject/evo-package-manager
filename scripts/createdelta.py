...
def minsandmaxes():
    ...
    packages = map(lambda x: os.path.basename(x).split(".io")[0], set(glob.glob("*.io")) - set(glob.glob("*.delta.io")))
...
if __name__ == "__main__":
    ...
    mi, ma = minsandmaxes()
    for pkg in mi.keys():
        old_pkg = "%s-%s.io" % (pkg, str(mi[pkg]))
        new_pkg = "%s-%s.io" % (pkg, str(ma[pkg]))
        name, version = util.parse_package_name(pkg)

        if not old_pkg == new_pkg:
        # skip if same 
            if not os.path.exists("%s-%s-%s.delta.io" % (name, str(mi[pkg].build), str(ma[pkg].build))):
            # skip if delta exists
                print "%s --> Min: %s Max: %s \n %s-%s-%s.delta.io" % (pkg, old_pkg, new_pkg, name, str(mi[pkg].build), str(ma[pkg].build))
