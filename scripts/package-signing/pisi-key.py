...
GPG_CMD = 'gpg --ignore-time-conflict --no-options --no-default-keyring \
            --secret-keyring /etc/io/secring.gpg --trustdb-name /etc/io/trustdb.gpg'
...
def net_update():
    ...
    if not os.path.isdir("/var/lib/io/keyrings"):
        os.mkdir("/var/lib/io/keyrings")

    keyring = "/var/lib/io/keyrings/%s" ARCHIVE_KEYRING.split("/")[-1]
