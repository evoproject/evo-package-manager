...
def cleanPisis(clean, root = '/var/cache/io/packages'):
    # pisi packages
    list = map(lambda x: os.path.basename(x).split(".io")[0], glob.glob("%s/*.io" % root))
    list.sort()
    l = findUnneededFiles(list)
    doit(root, l, clean, ".io")

def cleanBuilds(clean, root = '/var/io'):
    # Build remnant
    list = []
