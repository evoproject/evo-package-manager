...
class FilesDBTestCase(testcase.TestCase):
    ...
    def testAddRemoveFiles(self):
        ...
        fileinfo1.path = "etc/io/io.conf"
        fileinfo2 = pisi.files.FileInfo()
        fileinfo2.path = "etc/io/mirrors.conf"
        
        files = pisi.files.Files()
        files.list.append(fileinfo1)
        files.list.append(fileinfo2)

        assert not self.filesdb.has_file("etc/io/io.conf")
        assert not self.filesdb.has_file("etc/io/mirrors.conf")

        self.filesdb.add_files("pisi", files)

        assert self.filesdb.has_file("etc/io/io.conf")
        assert self.filesdb.has_file("etc/io/mirrors.conf")

        pkg, path = self.filesdb.get_file("etc/io/io.conf")
        assert pkg == "pisi"
