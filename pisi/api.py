...
def locked(func):
    ...
    def wrapper(*__args,**__kw):
        ...
        try:
            ...
            if not ctx.locked:
                raise pisi.errors.AnotherInstanceError(_("Another instance of io is running. Only one instance is allowed."))
...
def index(dirs=None, output='io-index.xml',
          skip_sources=False, skip_signing=False,
          compression=0):
    """Accumulate io XML files in a directory, and write an index."""
    ...
    for repo_dir in dirs:
        ...
        ctx.ui.info(_('Building index of io files under %s') % repo_dir)
...
def clearCache(all=False):
    ...
    def removeAll(cacheDir):
        cached = glob.glob("%s/*.io" % cacheDir) + glob.glob("%s/*.part" % cacheDir)
        for pkg in cached:
            try:
                os.remove(pkg)
            except exceptions.OSError:
    ...
    pkgList = map(lambda x: os.path.basename(x).split(ctx.const.package_suffix)[0], glob.glob("%s/*.io" % cacheDir))
    if not all:
        # Cache limits from pisi.conf
        config = pisi.configfile.ConfigurationFile("/etc/io/io.conf")
