...
"""
io Configuration module is used for gathering and providing
regular io configurations.
"""
...
class Config(object):
    ...
    def __init__(self, options = Options()):
        ...
        if os.path.exists("/etc/io/io.conf"):
            self.values = pisi.configfile.ConfigurationFile("/etc/io/io.conf")
        else:
            self.values = pisi.configfile.ConfigurationFile("/usr/share/defaults/io/io.conf")
    ...
    def tmp_dir(self):
        ...
        if os.environ.has_key('USER'):
            userdir = self.subdir('/tmp/io-' + os.environ['USER'])
        else:
            userdir = self.subdir('/tmp/io-root')
        # check write access
